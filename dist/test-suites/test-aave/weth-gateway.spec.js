...
make_suite_1.makeSuite('Use native ETH at LendingPool via WETHGateway', (testEnv) => {
    const zero = ethers_1.BigNumber.from('0');
    const depositSize = utils_1.parseEther('5');
    const daiSize = utils_1.parseEther('10000');
    ...
    it('Withdraw WETH - Partial', async () => {
        ...
        const partialWithdraw = await contracts_helpers_1.convertToCurrencyDecimals(aWETH.address, '2');
        ...
        const { gasUsed: approveGas } = await misc_utils_1.waitForTx(approveTx);
        // Partial Withdraw and send native Ether to user
        const { gasUsed: withdrawGas } = await misc_utils_1.waitForTx(await wethGateway
            .connect(user.signer)
    ...
    it('Borrow stable WETH and Full Repay with ETH', async () => {
        ...
        const borrowSize = utils_1.parseEther('1');
        ...
        const stableDebtToken = await contracts_getters_1.getStableDebtToken(stableDebtTokenAddress);
        // Deposit 10000 DAI
        ...
        await misc_utils_1.waitForTx(await pool.connect(user.signer).borrow(weth.address, borrowSize, '1', '0', user.address));
        ...
        await misc_utils_1.waitForTx(await wethGateway
            .connect(user.signer)
    ...
    it('Borrow variable WETH and Full Repay with ETH', async () => {
        ...
        const varDebtToken = await contracts_getters_1.getVariableDebtToken(variableDebtTokenAddress);
        // Deposit with native ETH
        await wethGateway
            .connect(user.signer)
            .depositETH(pool.address, user.address, '0', { value: depositSize });
        ...
        expect(aTokensBalance).to.be.gte(depositSize);
        // Borrow WETH with WETH as collateral
        await misc_utils_1.waitForTx(await pool.connect(user.signer).borrow(weth.address, borrowSize, '2', '0', user.address));
    ...
    it('Borrow ETH via delegateApprove ETH and repays back', async () => {
        ...
        await misc_utils_1.waitForTx(await varDebtToken.connect(user.signer).approveDelegation(wethGateway.address, borrowSize));
        // Borrows ETH with WETH as collateral
        await misc_utils_1.waitForTx(await wethGateway.connect(user.signer).borrowETH(pool.address, borrowSize, '2', '0'));
        const debtBalance = await varDebtToken.balanceOf(user.address);
        expect(debtBalance).to.be.gt(zero);
        // Full Repay WETH loan with native ETH
        await misc_utils_1.waitForTx(await wethGateway
    ...
    it('Should revert if receiver function receives Ether if not WETH', async () => {
        ...
        const user = users[0];
        const amount = utils_1.parseEther('1');
    ...
    it('Owner can do emergency native ETH recovery', async () => {
        ...
        const selfdestructContract = await contracts_deployments_1.deploySelfdestructTransferMock();
        ...
        const { gasUsed } = await misc_utils_1.waitForTx(callTx);
