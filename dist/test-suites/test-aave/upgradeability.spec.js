...
make_suite_1.makeSuite('Upgradeability', (testEnv) => {
    ...
    before('deploying instances', async () => {
        ...
        const aTokenInstance = await contracts_deployments_1.deployMockAToken([
            pool.address,
            dai.address,
        ...
        const stableDebtTokenInstance = await contracts_deployments_1.deployMockStableDebtToken([
            pool.address,
            dai.address,
            constants_1.ZERO_ADDRESS,
        ...
        const variableDebtTokenInstance = await contracts_deployments_1.deployMockVariableDebtToken([
            pool.address,
            dai.address,
            constants_1.ZERO_ADDRESS,
    ...
    it('Tries to update the DAI Atoken implementation with a different address than the lendingPoolManager', async () => {
        ...
        const name = await (await contracts_getters_1.getAToken(newATokenAddress)).name();
        const symbol = await (await contracts_getters_1.getAToken(newATokenAddress)).symbol();
        ...
        await chai_1.expect(configurator.connect(users[1].signer).updateAToken(updateATokenInputParams)).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
    });
    it('Upgrades the DAI Atoken implementation ', async () => {
        ...
        chai_1.expect(tokenName).to.be.eq('Aave Interest bearing DAI updated', 'Invalid token name');
    });
    it('Tries to update the DAI Stable debt token implementation with a different address than the lendingPoolManager', async () => {
        const { dai, configurator, users } = testEnv;
        const name = await (await contracts_getters_1.getStableDebtToken(newStableTokenAddress)).name();
        const symbol = await (await contracts_getters_1.getStableDebtToken(newStableTokenAddress)).symbol();
        ...
        await chai_1.expect(configurator
    ...
    it('Upgrades the DAI stable debt token implementation ', async () => {
        ...
        const debtToken = await contracts_getters_1.getMockStableDebtToken(stableDebtTokenAddress);
        const tokenName = await debtToken.name();
        chai_1.expect(tokenName).to.be.eq('Aave stable debt bearing DAI updated', 'Invalid token name');
    });
    it('Tries to update the DAI variable debt token implementation with a different address than the lendingPoolManager', async () => {
        const { dai, configurator, users } = testEnv;
        const name = await (await contracts_getters_1.getVariableDebtToken(newVariableTokenAddress)).name();
        const symbol = await (await contracts_getters_1.getVariableDebtToken(newVariableTokenAddress)).symbol();
        const updateDebtTokenInput = {
            asset: dai.address,
            incentivesController: constants_1.ZERO_ADDRESS,
            name: name,
    ...
    it('Upgrades the DAI variable debt token implementation ', async () => {
        ...
        const debtToken = await contracts_getters_1.getMockVariableDebtToken(variableDebtTokenAddress);
        const tokenName = await debtToken.name();
        chai_1.expect(tokenName).to.be.eq('Aave variable debt bearing DAI updated', 'Invalid token name');
    });
});
