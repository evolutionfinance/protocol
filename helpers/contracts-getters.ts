import {
  EvolutionProtocolDataProviderFactory,
  EvolutionTokenFactory,
  ATokensAndRatesHelperFactory,
  EvolutionOracleFactory,
...
export const getAToken = async (address?: tEthereumAddress) =>
  await EvolutionTokenFactory.connect(
    address || (await getDb().get(`${eContractid.EvolutionToken}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );
...
export const getAaveProtocolDataProvider = async (address?: tEthereumAddress) =>
  await EvolutionProtocolDataProviderFactory.connect(
    address ||
      (await getDb().get(`${eContractid.EvolutionProtocolDataProvider}.${DRE.network.name}`).value())
        .address,
    await getFirstSigner()
  );
...
export const getMockAToken = async (address?: tEthereumAddress) =>
  await MockETokenFactory.connect(
    address || (await getDb().get(`${eContractid.MockEToken}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );
...
export const getAaveOracle = async (address?: tEthereumAddress) =>
  await EvolutionOracleFactory.connect(
    address || (await getDb().get(`${eContractid.EvolutionOracle}.${DRE.network.name}`).value()).address,
    await getFirstSigner()
  );
