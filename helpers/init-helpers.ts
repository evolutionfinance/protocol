...
import { EvolutionProtocolDataProvider } from '../types/EvolutionProtocolDataProvider';
...
export const chooseATokenDeployment = (id: eContractid) => {
  switch (id) {
    case eContractid.EvolutionToken:
...
export const initReservesByHelper = async (
  ...
  const reserves = Object.entries(reservesParams).filter(
    ([_, { aTokenImpl }]) =>
      aTokenImpl === eContractid.DelegationAwareAToken || aTokenImpl === eContractid.EvolutionToken
  ) as [string, IReserveParams][];
  ...
  for (let [symbol, params] of reserves) {
    ...
    if (aTokenImpl === eContractid.EvolutionToken) {
  ...
  for (let chunkIndex = 0; chunkIndex < chunkedInitInputParams.length; chunkIndex++) {
    console.log('Debug', chunkedInitInputParams[chunkIndex]);
...
export const configureReservesByHelper = async (
  reservesParams: iMultiPoolsAssets<IReserveParams>,
  tokenAddresses: { [symbol: string]: tEthereumAddress },
  helpers: EvolutionProtocolDataProvider,
  admin: tEthereumAddress
) => {
  const addressProvider = await getLendingPoolAddressesProvider();
...
export const initTokenReservesByHelper = async (
    ...
    if (!stableTokenImpl) {
      const stableDebt = await deployStableDebtToken(
        [
          ...
          `Evolution stable debt bearing ${symbol}`,
          `stableDebt${symbol}`,
        ],
        verify
      );
    ...
    if (!variableTokenImpl) {
      const variableDebt = await deployVariableDebtToken(
        [
          ...
          tokenAddresses[symbol],
          ZERO_ADDRESS, // Incentives Controller
          `Evolution variable debt bearing ${symbol}`,
          `variableDebt${symbol}`,
        ],
        verify
      );
    ...
    if (!aTokenImplementation) {
      ...
      const aToken = await deployCustomAToken(
        [
          ...
          treasuryAddress,
          ZERO_ADDRESS,
          `Evolution interest bearing ${symbol}`,
          `a${symbol}`,
        ],
        verify
      );
  ...
  for (let i = 0; i < deployedATokens.length; i++) {
    initInputParams.push({
      ...
      incentivesController: ZERO_ADDRESS,
      underlyingAssetName: reserveSymbols[i],
      aTokenName: `Evolution interest bearing ${reserveSymbols[i]}`,
      aTokenSymbol: `e${reserveSymbols[i]}`,
      variableDebtTokenName: `Evolution variable debt bearing ${reserveSymbols[i]}`,
      variableDebtTokenSymbol: `variableDebt${reserveSymbols[i]}`,
      stableDebtTokenName: `Evolution stable debt bearing ${reserveSymbols[i]}`,
